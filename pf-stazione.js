...
async function caricaFoglioGiornaliero() {
  ...
  var [pompeRes,lettOggiRes,lettIeriRes,prezziRes,cassaRes,speseRes] = await Promise.all([
    sb.from('stazione_pompe').select('id,nome,prodotto,ordine_fisso').eq('attiva',true).order('ordine_fisso'),
    sb.from('stazione_letture').select('*').eq('data',data),
    sb.from('stazione_letture').select('*').lte('data',giornoPre).order('data',{ascending:false}).limit(50),
    sb.from('stazione_prezzi').select('*').eq('data',data),
    sb.from('stazione_cassa').select('*').eq('data',data).maybeSingle(),
    sb.from('stazione_spese_contanti').select('*').eq('data',data).order('created_at')
  ]);
  var pompe=pompeRes.data||[], lettOggi=lettOggiRes.data||[], lettIeri=lettIeriRes.data||[];
  var prezzi=prezziRes.data||[], cassa=cassaRes.data, spese=speseRes.data||[];

  var prezzoMap={}; prezzi.forEach(function(p){prezzoMap[p.prodotto]=Number(p.prezzo_litro);});
  var ieriMap={}; pompe.forEach(function(p){var t=lettIeri.find(function(l){return l.pompa_id===p.id;});if(t)ieriMap[p.id]=Number(t.lettura);});
  var oggiMap={}; lettOggi.forEach(function(l){oggiMap[l.pompa_id]=l;});

  // Pompe tabella orizzontale
  var litriPerProdotto={}, totLitri=0, totEuro=0;
  var thCols='',trIeri='',trOggi='',trDiff='';
  pompe.forEach(function(p){
    var pi=cacheProdotti.find(function(pp){return pp.nome===p.prodotto;}); var col=pi?pi.colore:'#888';
    var lO=oggiMap[p.id]; var vO=lO?Number(lO.lettura):null; var vI=ieriMap[p.id];
    var litri=(vO!==null&&vI!==undefined)?Math.max(0,vO-vI):0;
    var prezzo=prezzoMap[p.prodotto]||0; var euro=litri*prezzo;
    totLitri+=litri; totEuro+=euro;
    if(!litriPerProdotto[p.prodotto])litriPerProdotto[p.prodotto]={litri:0,euro:0,prezzo:prezzo};
    litriPerProdotto[p.prodotto].litri+=litri; litriPerProdotto[p.prodotto].euro+=euro;
  ...
  document.getElementById('fg-pompe-tabella').innerHTML='<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px"><thead><tr><th style="padding:5px 6px;border:1px solid var(--border);text-align:left;background:var(--bg);font-size:8px"></th>'+thCols+'</tr></thead><tbody><tr><td style="padding:5px 6px;border:1px solid var(--border);font-weight:600;background:var(--bg)">Lettura gg. prima</td>'+trIeri+'</tr><tr><td style="padding:5px 6px;border:1px solid var(--border);font-weight:600;background:var(--bg)">Lettura oggi</td>'+trOggi+'</tr><tr style="background:#FDF3D0"><td style="padding:5px 6px;border:1px solid var(--border);font-weight:bold">Litri venduti</td>'+trDiff+'</tr></tbody></table></div>';
  ...
  Object.keys(litriPerProdotto).forEach(function(prod){var d2=litriPerProdotto[prod];var pi=cacheProdotti.find(function(pp){return pp.nome===prod;});var col=pi?pi.colore:'#888';rH+='<tr><td style="padding:5px 8px;border:1px solid var(--border);font-weight:600"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:'+col+';margin-right:4px;vertical-align:middle"></span>'+esc(prod)+'</td><td style="padding:5px 8px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right">'+_sep(d2.litri.toLocaleString('it-IT',{maximumFractionDigits:1}))+'</td><td style="padding:5px 8px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right">€ '+d2.prezzo.toFixed(3)+'</td><td style="padding:5px 8px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right;font-weight:bold">'+fmtE(d2.euro)+'</td></tr>';});
  ...
  // Carte
  var bk=cassa?Number(cassa.bancomat||0):0, nx=cassa?Number(cassa.carte_nexi||0):0, az=cassa?Number(cassa.carte_aziendali||0):0, tc=bk+nx+az;
  document.getElementById('fg-carte-auto').innerHTML='<table style="width:100%;border-collapse:collapse;font-size:10px"><tr><td style="padding:3px 5px;border:1px solid var(--border)">Bancomat</td><td style="padding:3px 5px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right;width:40%">'+fmtE(bk)+'</td></tr><tr><td style="padding:3px 5px;border:1px solid var(--border)">Nexi</td><td style="padding:3px 5px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right">'+fmtE(nx)+'</td></tr><tr><td style="padding:3px 5px;border:1px solid var(--border)">Carte aziendali</td><td style="padding:3px 5px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right">'+fmtE(az)+'</td></tr><tr style="background:#E6F1FB;font-weight:bold"><td style="padding:4px 5px;border:1px solid var(--border)">Totale</td><td style="padding:4px 5px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right;color:#0C447C">'+fmtE(tc)+'</td></tr></table>';

  // Crediti
  var ce=cassa?Number(cassa.crediti_emessi||0):0, ri=cassa?Number(cassa.rimborsi_effettuati||0):0, rp=cassa?Number(cassa.rimborsi_giorni_prec||0):0, sc=ce-ri-rp;
  document.getElementById('fg-crediti-auto').innerHTML='<table style="width:100%;border-collapse:collapse;font-size:10px"><tr><td style="padding:3px 5px;border:1px solid var(--border)">Crediti emessi</td><td style="padding:3px 5px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right;width:40%;color:#639922">+ '+fmtE(ce)+'</td></tr><tr><td style="padding:3px 5px;border:1px solid var(--border)">Rimborsi</td><td style="padding:3px 5px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right;color:#A32D2D">− '+fmtE(ri)+'</td></tr><tr><td style="padding:3px 5px;border:1px solid var(--border)">Rimb. gg prec.</td><td style="padding:3px 5px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right;color:#A32D2D">− '+fmtE(rp)+'</td></tr><tr style="background:#FAEEDA;font-weight:bold"><td style="padding:4px 5px;border:1px solid var(--border)">Saldo</td><td style="padding:4px 5px;border:1px solid var(--border);font-family:var(--font-mono);text-align:right;color:#633806">'+(sc>=0?'+ ':'− ')+fmtE(Math.abs(sc))+'</td></tr></table>';

  // Spese
  var ls=document.getElementById('fg-spese-lista'); ls.innerHTML='';
  spese.forEach(function(s){_fgRigaSpesa(s.nota||'',Number(s.importo||0));});
  if(!spese.length)_fgRigaSpesa('',0);

  // Banconote
  [100,50,20,10,5,2,1].forEach(function(t){var el=document.getElementById('fg-b'+t);if(el)el.value=cassa?(Number(cassa['banconote_'+t])||0):0;});
  var me=document.getElementById('fg-monete');if(me)me.value=cassa?(Number(cassa.monete_varie)||0):0;

  _fgDati={totEuro:totEuro,totCarte:tc,bancomat:bk,nexi:nx,aziendali:az,creditiEmessi:ce,rimborsi:ri,rimborsiPrec:rp,litriPerProdotto:litriPerProdotto,pompe:pompe,ieriMap:ieriMap,oggiMap:oggiMap,prezzoMap:prezzoMap};
  fgCalcola();
}

function _fgRigaSpesa(nota,importo){
  var l=document.getElementById('fg-spese-lista');var div=document.createElement('div');
  div.style.cssText='display:flex;gap:6px;align-items:center;margin-bottom:4px';
  div.innerHTML='<input type="text" class="fg-spesa-nota" value="'+esc(nota)+'" placeholder="Descrizione spesa..." style="flex:1;font-size:12px;padding:6px 10px;border:0.5px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text)" /><input type="number" class="fg-spesa-importo" value="'+(importo||'')+'" placeholder="0.00" step="0.01" oninput="fgCalcola()" style="font-family:var(--font-mono);font-size:13px;text-align:right;width:100px;padding:6px 8px;border:0.5px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text)" /><button onclick="this.parentElement.remove();fgCalcola()" style="font-size:12px;padding:2px 8px;border:0.5px solid var(--border);border-radius:6px;background:var(--bg);cursor:pointer;color:#A32D2D">x</button>';
  l.appendChild(div);
}
...
function fgCalcola(){
  ...
  tagli.forEach(function(t){var n=parseInt(document.getElementById('fg-b'+t).value)||0;var tot=n*t;document.getElementById('fg-b'+t+'-tot').textContent='€ '+_sep(tot.toLocaleString('it-IT'));totC+=tot;});
  var mon=parseFloat(document.getElementById('fg-monete').value)||0;
  document.getElementById('fg-monete-tot').textContent='€ '+mon.toFixed(2);totC+=mon;
  document.getElementById('fg-contanti-totale').textContent=fmtE(totC);
  var totSp=0;document.querySelectorAll('.fg-spesa-importo').forEach(function(i){totSp+=parseFloat(i.value)||0;});
  var tV=_fgDati.totEuro||0,tCa=_fgDati.totCarte||0;
  ...
  var dCol=Math.abs(diff)<0.01?'#639922':'#E24B4A';var dBg=Math.abs(diff)<0.01?'#EAF3DE':'#FCEBEB';
...
async function salvaFoglioGiornaliero(){
  ...
  if(!data){toast('Seleziona una data');return;}
  var bn={};[100,50,20,10,5,2,1].forEach(function(t){bn['banconote_'+t]=parseInt(document.getElementById('fg-b'+t).value)||0;});
  ...
  if(ex){await sb.from('stazione_cassa').update(bn).eq('data',data);}
  else{bn.data=data;bn.totale_vendite=_fgDati.totEuro||0;await sb.from('stazione_cassa').insert([bn]);}
  await sb.from('stazione_spese_contanti').delete().eq('data',data);
  var sa=[];document.querySelectorAll('#fg-spese-lista > div').forEach(function(r){var n=r.querySelector('.fg-spesa-nota').value;var i=parseFloat(r.querySelector('.fg-spesa-importo').value)||0;if(i>0||n.trim())sa.push({data:data,nota:n,importo:i});});
...
async function stampaFoglioGiornaliero(){
  ...
  var d=_fgDati,pompe=d.pompe||[];
  var tagli=[100,50,20,10,5,2,1],totCont=0,tagliH='';
  tagli.forEach(function(t){var n=parseInt(document.getElementById('fg-b'+t).value)||0;var tot=n*t;totCont+=tot;tagliH+='<tr><td style="padding:2px 5px;border:1px solid #ccc;background:#f8f8f5;font-weight:500">€ '+t+'</td><td style="padding:2px 5px;border:1px solid #ccc;font-family:Courier New,monospace;text-align:center;width:20%">'+(n||'')+'</td><td style="padding:2px 5px;border:1px solid #ccc;font-family:Courier New,monospace;text-align:right;width:30%">€ '+tot.toFixed(2)+'</td></tr>';});
  ...
  var totSp=0,spH='';document.querySelectorAll('#fg-spese-lista > div').forEach(function(r){var n2=r.querySelector('.fg-spesa-nota').value||'—';var i=parseFloat(r.querySelector('.fg-spesa-importo').value)||0;if(i>0||n2.trim()){totSp+=i;spH+='<tr><td style="padding:3px 6px;border:1px solid #ccc">'+esc(n2)+'</td><td style="padding:3px 6px;border:1px solid #ccc;font-family:Courier New,monospace;text-align:right;color:#A32D2D">− € '+i.toFixed(2)+'</td></tr>';}});
  ...
  var contAtt=Math.max(0,d.totEuro-d.totCarte),crN=d.creditiEmessi-d.rimborsi-d.rimborsiPrec;
  var daV=Math.round((contAtt+crN-totSp)*100)/100,diff=Math.round((totCont-daV)*100)/100;
  var dC=Math.abs(diff)<0.01?'#639922':'#A32D2D',dB=Math.abs(diff)<0.01?'#EAF3DE':'#FCEBEB';
  // Pompe orizzontali
  var thP='',tI='',tO='',tD='';
  pompe.forEach(function(p){var pi=cacheProdotti.find(function(pp){return pp.nome===p.prodotto;});var col=pi?pi.colore:'#888';var lO=d.oggiMap[p.id];var vO=lO?Number(lO.lettura):null;var vI=d.ieriMap[p.id];var li=(vO!==null&&vI!==undefined)?Math.max(0,vO-vI):0;thP+='<th style="padding:4px;border:1px solid #C04A20;text-align:center;background:'+col+';color:#fff;font-size:7px;text-transform:uppercase">'+esc(p.prodotto)+'<br/>'+esc(p.nome)+'</th>';tI+='<td style="padding:4px;border:1px solid #ccc;font-family:Courier New,monospace;text-align:right;font-size:9px">'+(vI!==undefined?_sep(vI.toLocaleString('it-IT',{maximumFractionDigits:1})):'—')+'</td>';tO+='<td style="padding:4px;border:1px solid #ccc;font-family:Courier New,monospace;text-align:right;font-weight:bold;font-size:9px">'+(vO!==null?_sep(vO.toLocaleString('it-IT',{maximumFractionDigits:1})):'—')+'</td>';tD+='<td style="padding:4px;border:1px solid #ccc;font-family:Courier New,monospace;text-align:right;font-weight:bold;font-size:9px">'+(li>0?_sep(li.toLocaleString('it-IT',{maximumFractionDigits:1})):'—')+'</td>';});
  var rieH='';Object.keys(d.litriPerProdotto).forEach(function(pr){var pp=d.litriPerProdotto[pr];var pi=cacheProdotti.find(function(p){return p.nome===pr;});var col=pi?pi.colore:'#888';rieH+='<tr><td style="padding:4px 6px;border:1px solid #ccc;font-weight:600"><span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:'+col+';margin-right:3px"></span>'+esc(pr)+'</td><td style="padding:4px 6px;border:1px solid #ccc;font-family:Courier New,monospace;text-align:right">'+_sep(pp.litri.toLocaleString('it-IT',{maximumFractionDigits:1}))+'</td><td style="padding:4px 6px;border:1px solid #ccc;font-family:Courier New,monospace;text-align:right">€ '+pp.prezzo.toFixed(3)+'</td><td style="padding:4px 6px;border:1px solid #ccc;font-family:Courier New,monospace;text-align:right;font-weight:bold">€ '+_sep(pp.euro.toLocaleString('it-IT',{minimumFractionDigits:2,maximumFractionDigits:2}))+'</td></tr>';});
  var h='<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Foglio Giornaliero '+data+'</title><style>body{font-family:Arial,sans-serif;font-size:10px;margin:0;padding:10mm;color:#1a1a18}@media print{.no-print{display:none!important}@page{size:portrait;margin:8mm}}@media(max-width:600px){body{padding:4mm;font-size:9px}.fg-grid{grid-template-columns:1fr!important}}table{width:100%;border-collapse:collapse}.sect{font-size:10px;font-weight:bold;color:#D85A30;text-transform:uppercase;letter-spacing:0.4px;border-bottom:2px solid #D85A30;padding-bottom:3px;margin:10px 0 5px}.m{font-family:Courier New,monospace;text-align:right}</style></head><body>';
  ...
  h+='<div class="sect">✅ Quadratura giornata</div><table><tr><td style="padding:4px 6px;border:1px solid #ccc">Totale vendite</td><td class="m" style="padding:4px 6px;border:1px solid #ccc;font-weight:bold">€ '+_sep(d.totEuro.toLocaleString('it-IT',{minimumFractionDigits:2,maximumFractionDigits:2}))+'</td></tr><tr><td style="padding:4px 6px;border:1px solid #ccc">Totale carte</td><td class="m" style="padding:4px 6px;border:1px solid #ccc">€ '+d.totCarte.toFixed(2)+'</td></tr><tr><td style="padding:4px 6px;border:1px solid #ccc;font-weight:600">Contanti attesi</td><td class="m" style="padding:4px 6px;border:1px solid #ccc;font-weight:bold">€ '+contAtt.toFixed(2)+'</td></tr><tr style="background:#EAF3DE"><td style="padding:5px 6px;border:1px solid #ccc;font-weight:bold;font-size:11px">Da versare</td><td class="m" style="padding:5px 6px;border:1px solid #ccc;font-weight:bold;font-size:12px;color:#639922">€ '+daV.toFixed(2)+'</td></tr><tr><td style="padding:4px 6px;border:1px solid #ccc;font-weight:600">Contanti contati</td><td class="m" style="padding:4px 6px;border:1px solid #ccc;font-weight:bold">€ '+_sep(totCont.toLocaleString('it-IT',{minimumFractionDigits:2,maximumFractionDigits:2}))+'</td></tr><tr style="background:'+dB+'"><td style="padding:5px 6px;border:1px solid #ccc;font-weight:bold;font-size:11px">Differenza</td><td class="m" style="padding:5px 6px;border:1px solid #ccc;font-weight:bold;font-size:12px;color:'+dC+'">'+(diff>=0?'+':'−')+' € '+Math.abs(diff).toFixed(2)+'</td></tr></table>';
