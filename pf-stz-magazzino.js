...
async function caricaGiacenzeStazione() {
  ...
  if (cisterne && cisterne.length) {
    ...
    Object.entries(perProdotto).forEach(([prodNome, gruppo]) => {
      ...
      const cmpLabel = cmpGruppo > 0 ? '<div class="cmp-riga">CMP: <strong style="font-family:var(--font-mono);color:#A32D2D">€ ' + cmpGruppo.toFixed(6) + '</strong> · Valore: <strong style="font-family:var(--font-mono)">' + fmtE(totG * cmpGruppo) + '</strong>' + cmpEditBtn + cmpAnalisiBtn + '</div>' : '';
