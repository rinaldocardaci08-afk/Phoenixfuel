...
async function caricaGiacenzeStazione() {
  ...
  if (cisterne && cisterne.length) {
    ...
    Object.entries(perProdotto).forEach(([prodNome, gruppo]) => {
      ...
      var cmpCalcBtn = ' <button onclick="_stzApriCalcoloPrezzi(\'' + esc(prodNome) + '\')" style="font-size:9px;padding:1px 6px;background:none;border:0.5px solid var(--border);border-radius:4px;cursor:pointer;color:var(--text-muted)" title="Calcolo CMP e prezzi">🧮 Calcolo CMP e Prezzi</button>';
