...
async function caricaGiacenzeStazione() {
  ...
  // Allinea livello_attuale al calcolato (apertura + ricezioni - vendite letture
  // + rettifiche). Così il pannello mostra sempre dati canonici, senza che
  // l'utente debba premere "⚖️ Allinea cisterne".
  try {
    var prodRes = await sb.from('cisterne').select('prodotto').eq('sede','stazione_oppido');
    var prodSet = {};
  ...
  if (cisterne && cisterne.length) {
    ...
    Object.entries(perProdotto).forEach(([prodNome, gruppo]) => {
      ...
      var cmpEditBtn = puoModificareCmp ? ' <button onclick="_apriModificaCMP(\'' + esc(prodNome) + '\',\'' + gruppo.map(function(c){return c.id;}).join(',') + '\',' + totG + ',' + cmpGruppo.toFixed(6) + ',\'stazione_oppido\')" style="font-size:9px;padding:1px 6px;background:none;border:0.5px solid var(--border);border-radius:4px;cursor:pointer;color:var(--text-muted)" title="Modifica CMP">✏️</button>' : '';
      ...
      // v20260817a — Calcolo CMP e prezzi: simulatore su carichi non ancora entrati (pf-stz-prezzi.js).
      var cmpCalcBtn = ' <button onclick="_stzApriCalcoloPrezzi(\'' + esc(prodNome) + '\')" style="font-size:13px;padding:6px 14px;background:#185FA5;border:0.5px solid #185FA5;border-radius:6px;cursor:pointer;color:#fff;font-weight:500" title="Calcolo CMP e prezzi">🧮 Calcolo CMP e Prezzi</button>';
      const cmpLabel = cmpGruppo > 0 ? '<div class="cmp-riga">CMP: <strong style="font-family:var(--font-mono);color:#A32D2D">€ ' + cmpGruppo.toFixed(6) + '</strong> · Valore: <strong style="font-family:var(--font-mono)">' + fmtE(totG * cmpGruppo) + '</strong>' + cmpEditBtn + cmpAnalisiBtn + cmpCalcBtn + '</div>' : '';
