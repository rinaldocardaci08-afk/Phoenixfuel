...
async function caricaGiacenzeMensiliDeposito() {
  ...
  var prodotti = Object.keys(prodottiSet).sort(function(a,b){
    var ia = a.toLowerCase().indexOf('gasolio autotrazione') >= 0 ? 0 :
             a.toLowerCase().indexOf('gasolio agricolo') >= 0 ? 1 :
             a.toLowerCase().indexOf('benzina') >= 0 ? 2 : 3;
    var ib = b.toLowerCase().indexOf('gasolio autotrazione') >= 0 ? 0 :
             b.toLowerCase().indexOf('gasolio agricolo') >= 0 ? 1 :
             b.toLowerCase().indexOf('benzina') >= 0 ? 2 : 3;
    return ia - ib;
  });
...
function _depGmRenderTotali() {
  if (!_depGmDati) return '';
  var html = '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:11px"><thead><tr>';
  html += '<th style="text-align:left;padding:6px 8px;border:0.5px solid var(--border);background:var(--bg)">Prodotto</th>';
  _depGmMesi.forEach(function(m){ html += '<th style="text-align:right;padding:6px 8px;border:0.5px solid var(--border);background:var(--bg);font-size:9px">'+m.substring(0,3)+'</th>'; });
  html += '<th style="text-align:right;padding:6px 8px;border:0.5px solid var(--border);background:var(--bg)">Totale</th>';
  html += '</tr></thead><tbody>';

  _depGmDati.prodotti.forEach(function(prod){
    var dati = _depGmDati.mesi[prod];
    var totEnt = 0, totUsc = 0;
    html += '<tr><td style="padding:5px 8px;border:0.5px solid var(--border);font-weight:600">'+esc(prod)+'</td>';
    dati.forEach(function(d){
      totEnt += d.entrate; totUsc += d.uscite;
      var col = d.giacRilevata !== null ? '#2E7D32' : 'var(--text-muted)';
      var val = d.giacRilevata !== null ? d.giacRilevata : d.giacPresunta;
      html += '<td style="text-align:right;padding:5px 8px;border:0.5px solid var(--border);font-family:var(--font-mono);font-size:10px;color:'+col+'">'+Math.round(val).toLocaleString('it-IT')+'</td>';
    });
    html += '<td style="text-align:right;padding:5px 8px;border:0.5px solid var(--border);font-family:var(--font-mono);font-weight:700">'+Math.round(totEnt-totUsc).toLocaleString('it-IT')+' L</td>';
    html += '</tr>';
  });
