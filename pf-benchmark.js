...
async function caricaBenchmark() {
  ...
  kpiWrap.innerHTML =
    '<div class="kpi"><div class="kpi-label">Benchmark ' + prodotto.split(' ')[0] + '</div><div class="kpi-value" style="font-family:var(--font-mono)">€ ' + benchOggi.toFixed(6) + '</div></div>' +
    '<div class="kpi"><div class="kpi-label">Tuo CMP deposito</div><div class="kpi-value" style="font-family:var(--font-mono);color:' + (vantaggioAcq > 0 ? '#639922' : '#E24B4A') + '">€ ' + cmpCorrente.toFixed(6) + '</div><div style="font-size:10px;color:' + (vantaggioAcq > 0 ? '#639922' : '#E24B4A') + ';margin-top:2px">' + (vantaggioAcq > 0 ? '−' : '+') + '€ ' + Math.abs(vantaggioAcq).toFixed(6) + ' ' + (vantaggioAcq > 0 ? 'sotto' : 'sopra') + ' mercato</div></div>' +
    '<div class="kpi"><div class="kpi-label">Prezzo vendita medio</div><div class="kpi-value" style="font-family:var(--font-mono)">€ ' + prezzoVenditaMedio.toFixed(6) + '</div><div style="font-size:10px;color:' + (markupVendita >= 0 ? '#639922' : '#E24B4A') + ';margin-top:2px">' + (markupVendita >= 0 ? '+' : '') + '€ ' + markupVendita.toFixed(6) + ' su benchmark</div></div>' +
    '<div class="kpi" style="background:' + (spread >= 0.03 ? '#EAF3DE' : '#FCEBEB') + ';border:0.5px solid ' + (spread >= 0.03 ? '#639922' : '#E24B4A') + '"><div class="kpi-label" style="color:' + (spread >= 0.03 ? '#27500A' : '#791F1F') + '">Spread totale</div><div class="kpi-value" style="font-family:var(--font-mono);color:' + (spread >= 0.03 ? '#3B6D11' : '#E24B4A') + '">€ ' + spread.toFixed(6) + '/L</div><div style="font-size:10px;color:' + (spread >= 0.03 ? '#3B6D11' : '#E24B4A') + ';margin-top:2px">' + (prezzoVenditaMedio > 0 ? (spread / prezzoVenditaMedio * 100).toFixed(1) + '% su vendita' : '') + '</div></div>';
  ...
  if (cmpCorrente > 0 && prezzoVenditaMedio > 0) {
    ...
    posWrap.innerHTML = '<div style="font-size:11px;font-weight:500;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:14px">Posizionamento prezzo — ' + prodotto + '</div>' +
      ...
      '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">' +
        '<div style="padding:8px 12px;border-left:3px solid ' + (vantaggioAcq >= 0 ? '#639922' : '#E24B4A') + ';background:var(--bg);border-radius:0 8px 8px 0"><div style="font-size:9px;color:var(--text-muted);text-transform:uppercase">Vantaggio acquisto</div><div style="font-size:15px;font-weight:600;color:' + (vantaggioAcq >= 0 ? '#639922' : '#E24B4A') + ';font-family:var(--font-mono)">' + (vantaggioAcq >= 0 ? '−' : '+') + '€ ' + Math.abs(vantaggioAcq).toFixed(6) + '/L</div></div>' +
        '<div style="padding:8px 12px;border-left:3px solid ' + (markupVendita >= 0 ? '#378ADD' : '#E24B4A') + ';background:var(--bg);border-radius:0 8px 8px 0"><div style="font-size:9px;color:var(--text-muted);text-transform:uppercase">Markup su benchmark</div><div style="font-size:15px;font-weight:600;color:' + (markupVendita >= 0 ? '#378ADD' : '#E24B4A') + ';font-family:var(--font-mono)">' + (markupVendita >= 0 ? '+' : '') + '€ ' + markupVendita.toFixed(6) + '/L</div></div>' +
        '<div style="padding:8px 12px;border-left:3px solid ' + (spread >= 0.03 ? '#6B5FCC' : '#E24B4A') + ';background:var(--bg);border-radius:0 8px 8px 0"><div style="font-size:9px;color:var(--text-muted);text-transform:uppercase">Spread totale</div><div style="font-size:15px;font-weight:600;color:' + (spread >= 0.03 ? '#6B5FCC' : '#E24B4A') + ';font-family:var(--font-mono)">€ ' + spread.toFixed(6) + '/L</div></div>' +
      '</div>';
  } else {
  ...
  if (trendDir === 'salita') {
    ...
    segnaleColor = '#E24B4A';
    if (vantaggioAcq > 0) {
      consiglio = 'Il tuo CMP è ancora sotto il benchmark di € ' + vantaggioAcq.toFixed(6) + '/L — hai un vantaggio temporaneo. Conviene anticipare gli acquisti prima che il CMP si adegui al rialzo. Se hai giacenza sufficiente in deposito, usa il prodotto da magazzino finché il mercato non si stabilizza.';
    } else {
    ...
    segnaleColor = '#639922';
    consiglio = 'Il mercato è in calo. Conviene ritardare i nuovi acquisti per beneficiare di prezzi più bassi nei prossimi giorni. Usa il prodotto da magazzino per le consegne correnti. Spread attuale a € ' + spread.toFixed(6) + '/L — ' + (spread >= 0.03 ? 'margine sano.' : 'attenzione, margine basso!');
  } else {
    segnale = 'MERCATO STABILE — acquista secondo il piano normale';
    segnaleColor = '#BA7517';
    consiglio = 'Il mercato è stabile. Acquista secondo il piano normale. ' + (vantaggioAcq > 0 ? 'Il tuo CMP è sotto il benchmark di € ' + vantaggioAcq.toFixed(6) + '/L — buon posizionamento.' : 'Il CMP è allineato al mercato.') + ' Spread a € ' + spread.toFixed(6) + '/L — ' + (spread >= 0.03 ? 'margine nella norma.' : 'margine sotto soglia, valuta un ritocco prezzi.');
  }

  // Alert soglia spread
  var soglia = parseFloat(document.getElementById('bench-soglia')?.value) || 0.030;
  if (spread > 0 && spread < soglia) {
    inviaAvvisoSistema('⚠ Spread ' + prodotto + ' a € ' + spread.toFixed(6) + '/L — sotto soglia minima € ' + soglia.toFixed(3), 'sistema');
  }
  ...
  tbody.innerHTML = righe.map(function(b, idx) {
    ...
    return '<tr' + (idx % 2 ? ' style="background:var(--bg)"' : '') + '>' +
      ...
      '<td style="text-align:right;font-family:var(--font-mono);font-size:10px">' + (ma30Val ? ma30Val.toFixed(6) : '—') + '</td>' +
...
async function _calcolaBenchmarkDaMedia(data, mostraToast) {
  ...
  results.forEach(function(r) {
    if (!r.error) {
      salvati++;
      dettagli.push(r.prodotto + ': € ' + r.media.toFixed(6) + ' (media ' + r.count + ' fornitori)');
    }
  });
