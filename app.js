...
async function caricaSelectFornitori(selectId) {
  if (!cacheFornitori.length) { const { data } = await sb.from('fornitori').select('id,nome').eq('attivo',true).order('nome'); cacheFornitori = data||[]; }
...
async function caricaSelectClienti(selectId) {
  if (!cacheClienti.length) { const { data } = await sb.from('clienti').select('id,nome').eq('attivo',true).order('nome'); cacheClienti = data||[]; }
  const sel = document.getElementById(selectId); if (!sel) return;
...
async function salvaPrezzo() {
  ...
  if (!record.data||!record.fornitore||!record.prodotto||isNaN(record.costo_litro)) { toast('⚠ Compila tutti i campi'); return; }
...
async function caricaPrezzi() {
  ...
  tbody.innerHTML = data.map(r=>{
    ...
    return `<tr>
      ...
      <td>${best[k]&&best[k].id===r.id?'<span class="badge green" style="font-size:9px">Best</span>':''} <button class="btn-danger" onclick="eliminaRecord('prezzi','${r.id}',caricaPrezzi)">×</button></td>
    </tr>`;
  }).join('');
}

// ── ORDINI — selezioni filtrate per prezzi del giorno ─────────────
...
async function aggiornaSelezioniOrdine() {
  ...
  if (!data) return;
  // Carica solo i fornitori che hanno prezzi per quel giorno
...
function aggiornaBasiOrdine() {
  ...
  const basi = [...new Map(prezziDelGiorno.filter(p=>p.fornitore===fornitore&&p.basi_carico).map(p=>[p.basi_carico.id, p.basi_carico])).values()];
...
function aggiornaProdottiOrdine() {
  ...
  const prodotti = [...new Set(prezziDelGiorno.filter(p=>p.fornitore===fornitore&&(p.base_carico_id===baseId||(!p.base_carico_id&&!baseId))).map(p=>p.prodotto))];
...
async function caricaPrezzoPerOrdine() {
  // aggiorna prodotti disponibili prima
  aggiornaProdottiOrdine();
  const fornitore = document.getElementById('ord-fornitore').value;
  const baseId = document.getElementById('ord-base').value;
  const prodotto = document.getElementById('ord-prodotto').value;
  if (!fornitore||!prodotto) return;
  const match = prezziDelGiorno.find(p=>p.fornitore===fornitore&&p.prodotto===prodotto&&(p.base_carico_id===baseId||(!baseId&&!p.base_carico_id)));
  if (match) {
...
async function salvaOrdine() {
  if (!prezzoCorrente) { toast('⚠ Seleziona data/fornitore/base/prodotto disponibili'); return; }
...
// ── DEPOSITO ──────────────────────────────────────────────────────
const DEP_CONFIG = {
  autotrazione: { label:'Gasolio Autotrazione', colore:'#D85A30', elId:'dep-autotrazione', totId:'dep-total-autotrazione' },
  agricolo:     { label:'Gasolio Agricolo',     colore:'#639922', elId:'dep-agricolo',      totId:'dep-total-agricolo' },
  hvo:          { label:'HVO',                  colore:'#3B6D11', elId:'dep-hvo',           totId:'dep-total-hvo' },
  benzina:      { label:'Benzina',              colore:'#378ADD', elId:'dep-benzina',        totId:'dep-total-benzina' }
};

function cisternasvg(pct, colore) {
  const altezzaMax = 80;
  const livello = Math.round((pct/100)*altezzaMax);
  const y = 10 + (altezzaMax - livello);
  const fillColore = pct < 20 ? '#E24B4A' : pct < 35 ? '#BA7517' : colore;
  return `<svg class="dep-cisterna-svg" viewBox="0 0 60 100" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="5" y="${y}" width="50" height="${livello}" rx="2" fill="${fillColore}" opacity="0.85"/>
...
async function caricaDeposito() {
  ...
  Object.entries(DEP_CONFIG).forEach(([tipo, cfg]) => {
    ...
    el.innerHTML = gruppo.map(c=>{
      ...
      totGruppo += Number(c.livello_attuale);
      if (pct < 30) allerte++;
    ...
    document.getElementById(cfg.totId).textContent = fmtL(totGruppo);
    totaleStoccato += totGruppo;
  });
  ...
  // Movimenti recenti (ordini deposito + approvvigionamenti)
  const { data: movimenti } = await sb.from('ordini').select('*').or('tipo_ordine.eq.deposito,cliente.eq.Deposito').order('created_at',{ascending:false}).limit(10);
  const tbody = document.getElementById('dep-movimenti');
  if (!movimenti||!movimenti.length) { tbody.innerHTML='<tr><td colspan="6" class="loading">Nessun movimento</td></tr>'; return; }
  tbody.innerHTML = movimenti.map(r=>{
    const tipoMov = r.tipo_ordine==='deposito' ? '<span class="badge teal">Entrata</span>' : '<span class="badge amber">Uscita</span>';
    return `<tr><td>${r.data}</td><td>${tipoMov}</td><td>${r.prodotto}</td><td style="font-family:var(--font-mono)">${fmtL(r.litri)}</td><td>${r.fornitore}</td><td>${badgeStato(r.stato)}</td></tr>`;
  }).join('');
}
...
async function salvaCliente() {
  ...
  if (error) { toast('Errore: '+error.message); return; }
  toast('✅ Cliente salvato!'); cacheClienti=[];
  caricaClienti();
}
...
// ── FORNITORI ─────────────────────────────────────────────────────
async function caricaCheckboxBasi() {
  ...
  wrap.innerHTML = data ? data.map(b=>`<label class="check-label"><input type="checkbox" value="${b.id}" /> ${b.nome}</label>`).join('') : '';
}
...
async function salvaFornitore() {
  ...
  if (error) { toast('Errore: '+error.message); return; }
  // Salva basi associate
  const checks = document.querySelectorAll('#fo-basi-check input:checked');
  if (checks.length && fo) {
    const basi = Array.from(checks).map(c=>({ fornitore_id: fo.id, base_carico_id: c.value }));
    await sb.from('fornitori_basi').insert(basi);
  }
  toast('✅ Fornitore salvato!'); cacheFornitori=[];
  caricaFornitori();
}
